...
avg_order_value = filtered["revenue"].mean() if total_orders > 0 else 0
...
tabs = st.tabs(["Overview", "By Category", "By Region", "Data"])
...
with tabs[1]:
    ...
    if len(filtered) == 0:
        ...
        category_df = (
            filtered.groupby("category")["revenue"].sum().reset_index().sort_values("revenue")
        )
